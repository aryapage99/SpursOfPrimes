...
Original file is located at
    https://colab.research.google.com/drive/1P-HMU2ydDseEeAiyhM3kR5tdxJ8yd-vd
"""
...
def main():
    ...
    for fraction, semiprime_list in even_smaller_fractions.items():
        semiprime_list.sort()
        y_values = [1 / math.sqrt(factor) for factor in semiprime_list]  # New y-values
        plt.scatter(semiprime_list, y_values, color='blue', label=f"1 / sqrt(Prime Factor): {fraction:.2f}")
    plt.xlabel('Even Semiprime Numbers')
    plt.ylabel('1 / sqrt(Prime Factor)')
    plt.title('Even Semiprime Numbers vs. 1 / sqrt(Prime Factor)')
    plt.grid(True)
    #plt.legend()
    plt.show()

    # Generate the scatter plot for odd semiprimes (1 / sqrt(smaller prime factor))
    plt.figure()
    ...
    plt.ylabel('1 / sqrt(Prime Factor)')
    plt.title('Odd Semiprime Numbers vs. 1 / sqrt(Prime Factor)')
    plt.grid(True)
    #plt.legend()
    plt.show()
    ...
    for fraction, semiprime_list in even_smaller_fractions.items():
        semiprime_list.sort()
        y_values = [1 / math.sqrt(factor) for factor in semiprime_list]  # New y-values
        plt.plot(semiprime_list, y_values, 'bo', label=f"1 / sqrt(Prime Factor): {fraction:.2f}")
    plt.xlabel('Even Semiprime Numbers')
    plt.ylabel('1 / sqrt(Prime Factor)')
    plt.title('Even Semiprime Numbers vs. 1 / sqrt(Prime Factor)')
    plt.grid(True)
   # plt.legend()
    ...
    # Generate the line chart for odd semiprimes (1 / sqrt(smaller prime factor))
    plt.figure()
    for fraction, semiprime_list in odd_smaller_fractions.items():
        semiprime_list.sort()
        y_values = [1 / math.sqrt(factor) for factor in semiprime_list]  # New y-values
